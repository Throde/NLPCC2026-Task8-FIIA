...
对比例2中的两个句子可以发现，仅仅对认知动词和逻辑连词进行微小扰动（将“都知道”替换为“不知道”，将“但是”替换为“因为”），就足以导致大模型对目标分句“西部大开发需要资金和技术”的事实判断一致性急剧下降。这种判断上的不稳定性将在实际部署中引发严重的可靠性危机——如果同一个智能体在面对相同或高度相似的问题时，对同一事实的多次推理判断表现出高度的不稳定性，系统输出将变得不可信，这将导致其无法胜任容错成本高的下游应用，如司法事实抽取、医疗病历挖掘和教育辅助。
