...
1. **FIIA攻击语料及相关分析资源现已公开发布。** 本次发布包含6份经过筛选的较高分team-track提交语料，纳入标准为最终得分不低于60分。这些语料共包含 **1,200条攻击提交实例**，经攻击文本精确去重后对应 **1,190条不同的攻击文本**。同时发布的资源还包括从语言学角度构建的**17类语义分类体系**、基于大语言模型辅助判断的多标签语义标注、Qwen与DeepSeek的重复调用结果，以及逐条MIR得分。这些资源可用于响应一致性评估、汉语叙实性推理、对抗性语言改写、语言策略分析和跨模型行为比较等后续研究。相关资源均位于 [`submissions`](submissions/) 目录：
