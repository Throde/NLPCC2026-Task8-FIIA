...
1. **参赛队伍提交的攻击语料及相关分析资源现已公开发布。** 本次发布包含6份经过筛选的较高分team-track提交语料，纳入标准为最终得分不低于60分。这些语料共包含 **1,200条攻击提交实例**，经攻击文本精确去重后对应 **1,190条不同的攻击文本**。同时发布的资源还包括从语言学角度构建的**17类语义分类体系**、基于大语言模型辅助判断的多标签语义标注、Qwen与DeepSeek的重复调用结果，以及逐条MIR得分。这些资源可用于响应一致性评估、汉语叙实性推理、对抗性语言改写、语言策略分析和跨模型行为比较等后续研究。相关资源均位于 [`submissions`](submissions/) 目录：
   - [经过筛选的攻击提交语料](submissions/)
   - [元数据：原始提交与去重攻击语料之间的来源映射](submissions/metadata/)
   - [标注数据：语义分类体系及基于大语言模型判断的语义标注](submissions/annotations/)
   - [评估结果：重复调用结果与MIR评估数据](submissions/evaluation_results/)
2. 如需了解参赛队伍提交的具体攻击数据及相关标注与评估分析结果，或使用这些资源开展后续分析研究，请引用FIIA共享任务综述论文：
...
### 2026-07-02
1. 评测报告投稿系统现已开放，提交入口为：[NLPCC 2026 Shared Tasks OpenReview Submission](https://openreview.net/group?id=ccf.org/NLPCC/2026/Shared_Tasks)。请有意提交技术报告的队伍及时准备稿件并在截止时间前完成系统提交。评测报告截稿时间为 **2026年7月17日**。

### 2026-06-27
1. 本次评测任务的正式数据提交阶段现已结束，榜单结果可见：[评测结果](#评测结果)或[评测榜单网站](http://124.221.19.182/)。
2. 评测证书及技术报告提交安排已公布，请相关参赛队伍留意技术报告准备要求及后续提交通知。综合榜单排名靠前的 3 支队伍（ZZUNLP、FActProbe、NanceTide）原则上应提交评测报告。评测报告格式参照NLPCC主会论文标准：[NLPCC 2026 Calls](http://tcci.ccf.org.cn/conference/2026/calls/)，篇幅要求可适当放宽。
