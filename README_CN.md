...
### 2026-07-02
1. [获奖证书及技术报告提交安排](#获奖证书及技术报告提交安排)已更新，投稿系统已开放。评测报告截稿时间为 **2026 年 7 月 17 日**，请有意提交报告的队伍及时完成稿件准备与系统提交。

### 2026-06-27
1. 本次评测任务的正式数据提交阶段现已结束，榜单结果可见：[评测结果](#评测结果)或[评测榜单网站](http://124.221.19.182/)。
2. [获奖证书及技术报告提交安排](#获奖证书及技术报告提交安排)已公布，请相关参赛队伍留意技术报告准备要求及后续提交通知。
   
### 2026-04-30
1. 样本有效性检测工具已上线：参赛队伍可以访问[评测工具页](http://124.221.19.182/)使用“样本有效性检测”功能，随时将改编后的 JSON 文件上传至系统，系统会按照 R1–R5 规则逐条检测攻击样本的有效性，用于提交前的规范性准入预查。
2. [评价标准](#评价标准)已更新。
3. [提交要求](#提交要求)简化：最终提交的数据格式中，不再要求包含`response_original`和`response_attack`这两个字段。`validate.py`也做了相应修改。
...
本次评测任务的正式数据提交阶段现已结束，榜单结果可见：[评测榜单网站](http://124.221.19.182/)。

### Qwen 赛道最终排行榜

| 排名 | 队伍名称 | AttackScore | 有效攻击数 | 取得日期 |
|---:|---|---:|---:|---|
...
根据本次评测任务两个赛道的最终排行榜结果，每个赛道排名第一的队伍将获得由大会颁发的评测结果证书。具体获证队伍如下：

| 赛道 | 获证队伍 | 队伍负责人邮箱 |
|---|---|---|
| Qwen 赛道 | FActProbe | pengyiyang660@163.com |
| DeepSeek 赛道 | ZZUNLP | 1003773658@qq.com |

### 二、关于技术报告提交与遴选

根据大会组委会安排，本评测任务共有 3 篇评测报告邀请配额。评测报告旨在展示具有较高方法创新性、数据构造价值和模型分析价值的参赛成果。主办方将不单纯以最终榜单分数作为唯一依据，而是综合考量各队伍的攻击得分、提交数据集质量以及现象发现价值。

综合榜单排名靠前的 3 支队伍（ZZUNLP、FActProbe、NanceTide）原则上应提交评测报告；同时，欢迎其他参赛队伍根据自身意愿自愿提交技术报告。主办方将从所有提交的技术报告中进行综合评审。

评审时将主要参考以下几个方面：
1. **攻击得分表现**：队伍在相应赛道中的最终排行榜表现；
2. **数据集质量**：包括攻击样本设计的新颖性、多样性、自然性、可解释性，以及对计算语言学社区的潜在贡献价值；
3. **现象发现价值**：包括报告是否揭示了有意义的语言现象、事实性推理难点、模型不一致性弱点或其他值得进一步研究的问题。

主办方将本着宁缺毋滥的原则，在综合评审后择优确定最终推荐给大会学术委员会的 3 篇报告。最终正式收录的评测报告总量由 NLPCC 学术委员会审定。

评测报告格式参照 NLPCC 主会论文标准：[NLPCC 2026 Calls](http://tcci.ccf.org.cn/conference/2026/calls/)。篇幅要求可适当放宽，截稿时间为 **2026 年 7 月 17 日**。评测报告提交入口为：[NLPCC 2026 Shared Tasks OpenReview Submission](https://openreview.net/group?id=ccf.org/NLPCC/2026/Shared_Tasks)。请有意提交技术报告的队伍尽早准备稿件内容，并在截止时间前完成系统提交。
...
A：不需要。参赛队伍可自由选择样本进行改编，并提交经自测后攻击效果较好的样本。计入最终成绩的样本数最多为 200 条。

**Q3：我能否使用大模型或其他 AI 工具来协助生成攻击样本？**  
A：可以。关于攻击文本的生成方式，组织方不作限制。参赛队伍可以采用人工改写、规则程序辅助、大模型辅助（本任务指定的模型亦可使用）等方式生成候选攻击样本。

**Q4：我在改编一些文本后，想要及时测试其对指定大模型的攻击效果，产生的 API 费用由谁承担？**  
A：前期探索与测试阶段产生的 API 调用费用由参赛队伍自行承担。冲榜阶段（6月11日-6月20日）将开放评测系统后台（和排行榜），届时各队伍即可提交攻击数据集至后台，获取结果分数（此阶段的API费用由组织方承担）。

**Q5：具体什么样的改编句子，才算作一次成功的攻击？**  
A：简而言之，我们需要的改编句子要能使模型推理结果变得**更加混乱**，即只关心一致率变化情况，而不关心答案具体是什么。以一个具体例子来说明：假设我们选定数据集中的句子 A 作为改编对象，基于它改编出了 A1 和 A2 两个句子。

- 模型对原句 A 的10次判断结果为：`T,T,T,T,T,T,T,T,T,T`
- 模型对改编句 A1 的10次判断结果为：`U,U,U,U,U,U,U,U,U,U`
- 模型对改编句 A2 的10次判断结果为：`T,U,T,U,T,U,T,U,T,U`

对于改编句 A1，虽然模型的判断结果改变了，但是一致率和原句 A 相同（都是 100%），模型仍然逻辑自洽，因此句子 A1 是一次**无效的改编**。对于改编句 A2，一致率仅有50%，模型的判断结果不再稳定（每次询问都可能取得不同的答案），因此句子 A2 是一次**符合本任务目标的改编**。
...
| API 官网 | [Aliyun Bailian API Docs](https://bailian.console.aliyun.com/cn-beijing?spm=5176.12818093_47.overview_recent.1.e6402cc9lKf4ab&tab=api#/api) | [DeepSeek API Docs](https://api-docs.deepseek.com/zh-cn/) |
...
## 提交要求
参赛队伍需要将待提交的改编数据整理为 JSON 格式的输出文件。输出文件中的每条数据应包含`id`和`text_attack`两个字段，例如：
...
> 如果某支队伍实际改编了超过 200 条数据，并将这些数据作为攻击样本集全部提交给系统，系统将仅计算该数据集中前 200 条数据的不一致率得分作为最终成绩。
...
若数据项缺少必要的字段，或 id 非法/不存在/重复，该样本将被判定为无效。
...
参赛队伍完成改编文本后，可以将数据上传至[评测工具页](http://124.221.19.182/)，使用“样本有效性检测”功能进行自动检测。建议参赛队伍在数据改编过程中，及时使用自查程序对样本进行基础格式检查与有效性核查。

上述有效性检查规则所涉及的算法实现均已公开（`validate/validate.py`）。参赛队伍也可下载代码，在本地配置环境以自查数据。在后期冲榜阶段，评测系统后台使用的有效性判定算法将与公开发布的自查程序在逻辑上保持完全一致。


## 评价标准

本任务采用 **一致性攻击得分（AttackScore）** 作为最终排名依据。AttackScore 越高，表示攻击效果越强，最高为 120 分。该指标按以下三个步骤计算。

### 单条样本不一致率（MIR）

对于每条通过有效性检查的攻击样本，评测后台会分别对原句（`text_original`）和攻击句（`text_attack`）进行 10 轮模型调用。对于某一条样本，可以得到其不一致率（Multi-turn Inconsistency Rate, MIR）：

$$
MIR = 1 - \frac{\max(c_T, c_F, c_U)}{10}
$$

其中， $c_T$ 、 $c_F$ 、 $c_U$ 分别表示 10 轮调用中模型输出 `T`、`F`、`U` 的次数。例如，假设某样本的10轮调用结果为 `T,T,T,T,T,T, F,F,F, U` ，则最高频答案为 `T`，出现 6 次，因此该样本的不一致率为 $MIR = 1 - \frac{6}{10} = 0.4$ 。模型在多轮回答中的分歧越大，MIR 越高；在本任务中，MIR 的理论范围为： $MIR \in [0, 0.6]$ 。

### 单条样本攻击得分（ $Score_i$ ）

在此基础上，单条样本的攻击得分定义为 **攻击句不一致率（ $MIR_{attack}$ ）** 相对于 **原句不一致率（ $MIR_{orig}$ ）** 的增加量：

$$
Score_i = \max(MIR_{attack} - MIR_{orig}, 0)
$$

这意味着：只有当攻击句比原句引发了更高的不一致性时，该样本才会获得正分；如果攻击句没有提升模型回答的不一致性，则该样本得分为 0。 $Score_i$ 的理论范围为： $Score_i \in [0, 0.6]$ 。

### 攻击总得分（AttackScore）

最终攻击得分为所有有效样本攻击得分的累加。每支队伍每个赛道按提交文件中的顺序，至多计入前 200 条有效攻击样本：

$$
AttackScore = \sum_i Score_i
$$

由于单条样本攻击得分最高为 0.6，且最多计入 200 条有效攻击样本，因此 AttackScore 的理论范围为： $AttackScore \in [0, 120]$。

> 注意：每个原始样本 `id` 只允许提交一条攻击样本。若参赛队伍在同一提交文件中针对同一 `id` 提交多条改编样本，仅保留首次出现的样本参与评分，重复样本不参与最终评分。
